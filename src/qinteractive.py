...
ACTIONS = {Dir.UP: (0, -1), Dir.DOWN: (0, 1), Dir.LEFT: (-1, 0), Dir.RIGHT: (1, 0)}
...
def in_bound(newx, newy):
    return newx >= 0 and newx < maxx and newy >= 0 and newy < maxy
...
for i in range(episodes):
    ...
    while not terminated:
        ...
        grid[new_state[0], new_state[1]] = "O"
        stepcount += 1
        if reward == 1:
            rewards_per_episode[i] = stepcount
        q[state][action] = q[state][action] + learning_rate_a * (
            reward
            + discount_factor_g * np.max([v for k, v in q[new_state].items()])
            - q[state][action]
        )
        state = new_state
        time.sleep(1/2)
    ...
    if i % 100 == 0:
        print("Episodio:", i, ":",rewards_per_episode[i], "pasos")
